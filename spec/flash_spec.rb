...
describe Flash do
  ...
  let(:cook) do
    WEBrick::Cookie.new('_ruby_on_rails_lite_flash_', { xyz: 'abc' }.to_json)
  end
  ...
  describe "#store_flash" do
    context "without cookies in request" do
      ...
      it "adds new cookie with '_ruby_on_rails_lite_flash_' name to response" do
        cookie = res.cookies.find { |c| c.name == '_ruby_on_rails_lite_flash_' }
    ...
    context "with cookies in request" do
      before(:each) do
        cook = WEBrick::Cookie.new('_ruby_on_rails_lite_flash_', { pho: "soup" }.to_json)
        req.cookies << cook
      end
