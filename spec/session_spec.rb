...
describe Session do
  ...
  let(:cook) { WEBrick::Cookie.new('_ruby_on_rails_lite_', { xyz: 'abc' }.to_json) }
  ...
  describe "#store_session" do
    context "without cookies in request" do
      ...
      it "adds new cookie with '_ruby_on_rails_lite_' name to response" do
        cookie = res.cookies.find { |c| c.name == '_ruby_on_rails_lite_' }
    ...
    context "with cookies in request" do
      before(:each) do
        cook = WEBrick::Cookie.new('_ruby_on_rails_lite_', { pho: "soup" }.to_json)
        req.cookies << cook
      end
